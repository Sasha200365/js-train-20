...
Car.toString = function () {
  return `${this.brand} ${this.model} ${this.year} - ${fuelType}`;
};
...
Truck.specific = function () {
  if (this.specific > towingCapacity) {
...
const myTruck = Object.create(Truck, {
  brand: { value: "Toyota" },
  model: { value: "Tundra" },
  year: { value: 2019 },
  mileage: { value: 20000 },
  color: { value: "Red" },
  engineType: { value: "V8" },
  towingCapacity: { value: 10000 },
  fuelType: { value: "Gasoline" },
  transmissionType: { value: "Automatic" },
  doors: { value: 4 },
  weight: { value: 5600 },
});
/*
...
function tow() {
  return this.towingCapacity < towingCapacity;
}
...
const drive = car.bing(car, brand, model, year, mileage, fuelType, speed);
...
function ElectricCar(brand, model, year, mileage, batteryCapacity) {
  ...
  car.this.batteryCapacity = batteryCapacity;
}
...
ElectricCar.toString = function () {
  return `${rand} ${model} ${year} - Батарея: ${atteryCapacity} kWh`;
};
// Створюємо новий екземпляр ElectricCar
const ElectricCar = Object.create(Truck, {
  brand: { value: "Tesla" },
  model: { value: "Model S" },
  year: { value: 2020 },
  mileage: { value: 10000 },
...
tesla.toString = function () {
  console.log(this.tesla.toString);
};
